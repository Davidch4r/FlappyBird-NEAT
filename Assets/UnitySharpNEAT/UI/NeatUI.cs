    ...
    public class NeatUI : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            if (GUI.Button(new Rect(10, 10, 110, 40), "Start EA"))
            {
                ...
                GameObject.FindGameObjectWithTag("Spawner").GetComponent<PipeSpawnerScript>().start = true;
            }
            if (GUI.Button(new Rect(10, 60, 110, 40), "Stop + save EA"))
            {
                _neatSupervisor.StopEvolution();
                GameObject.FindGameObjectWithTag("Spawner").GetComponent<PipeSpawnerScript>().start = false;
            }
